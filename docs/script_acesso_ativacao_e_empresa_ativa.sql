-- =========================================================
-- SCRIPT DE ENTREGA — USR-001, ISOL-008 e a Etapa 1 da empresa ativa
--
-- Onde colar: SQL Editor do projeto (homologação primeiro; produção só depois
-- do aceite). Roda o arquivo INTEIRO de uma vez.
--
-- O QUE MUDA DE COMPORTAMENTO:
--   · quem tem conta de acesso ligada ao cadastro passa a ser carimbado como
--     ativo (inclusive um reparo dos que ficaram para trás — a cópia de
--     segurança é tirada antes, e o comando para desfazer fica no arquivo);
--   · cadastro em status de PRÉ-ativação deixa de ter acesso. Isto só é seguro
--     DEPOIS do carimbo acima, que vem no mesmo arquivo e nesta ordem;
--   · duas rotinas deixam de responder sobre colaborador de outro cliente
--     quando quem pergunta está logado (contexto interno segue como antes).
--
-- O QUE NÃO MUDA COMPORTAMENTO: toda a parte da empresa ativa (bloco 5). Ela
-- cria a tabela, a rotina de registro e a leitura, mas NADA decide acesso com
-- isso ainda. É de propósito: primeiro o seletor passa a registrar a escolha e
-- medimos a adoção; só então a regra aperta. Inverter a ordem trocaria um
-- vazamento por gente sem acesso.
--
-- Depois de aplicar, a medição que autoriza a próxima etapa:
--   SELECT * FROM public.empresa_ativa_adocao();
--
-- Origem: bateria do motor de QA do módulo Usuários & Permissões.
--
...
--
-- Esperado: carimbo_de_ativacao = 1, decisao_exige_ativo = 1,
-- rotinas_privilegiadas_corrigidas = 2, empresa_ativa_pronta = 3.
--
-- reparados_nesta_rodada mostra quantos cadastros estavam com conta ligada e
-- status para trás — eles foram carimbados como ativo, e a cópia está em
-- backup_usuarios_status_ativacao_20260916. Se o número surpreender, confira
-- a cópia ANTES de seguir para produção.
--
-- empresa_ativa_registradas começa em 0 e é ISSO MESMO: nesta etapa o banco só
-- passou a poder saber. O número sobe conforme as pessoas usarem o sistema, e
-- é ele que autoriza a próxima etapa. Acompanhe por:
--   SELECT * FROM public.empresa_ativa_adocao();
-- =========================================================
SELECT
  (SELECT count(*) FROM pg_trigger
    WHERE tgname = 'trg_usuario_ativa_ao_ligar_conta' AND NOT tgisinternal)   AS carimbo_de_ativacao,
  (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'perfil_permite_modulo'
      AND pg_get_functiondef(p.oid) ILIKE '%IS DISTINCT FROM ''ativo''%')     AS decisao_exige_ativo,
  (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname IN ('ponto_empresa_do_colaborador', 'colaborador_tem_vinculos')
      AND pg_get_functiondef(p.oid) ILIKE '%get_user_tenant_id%')             AS rotinas_privilegiadas_corrigidas,
  (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname IN ('definir_empresa_ativa', 'empresa_ativa', 'empresa_ativa_adocao'))
                                                                              AS empresa_ativa_pronta,
  (SELECT count(*) FROM public.usuarios_base
    WHERE auth_user_id IS NOT NULL
      AND status::text IN ('rascunho','pendente_convite','convite_enviado','aguardando_ativacao'))
                                                                              AS ainda_em_pre_ativacao,
  COALESCE((SELECT count(*) FROM public.backup_usuarios_status_ativacao_20260916), 0)
                                                                              AS reparados_nesta_rodada,
  (SELECT count(*) FROM public.usuario_empresa_ativa)                         AS empresa_ativa_registradas;
