-- =========================================================
-- SCRIPT DE ENTREGA — Empresa ativa, Etapa 2
--
-- Onde colar: SQL Editor do projeto (homologação primeiro; produção só depois
-- do aceite). Roda o arquivo INTEIRO de uma vez.
--
-- O QUE MUDA: quem tem empresa ativa registrada passa a ser avaliado pelos
-- vínculos e liberações DAQUELA empresa, não pela união de todas. Como as
-- políticas restritivas de perfil chamam a função que muda aqui, elas passam a
-- recortar por empresa sem terem sido tocadas.
--
-- O QUE NÃO MUDA AGORA: sessão sem empresa registrada continua decidindo como
-- antes. Essa tolerância é proposital e se fecha depois, por app_config, sem
-- nova migration — e volta atrás pelo mesmo caminho.
--
-- PRÉ-REQUISITO: o script da Etapa 1
-- (docs/script_acesso_ativacao_e_empresa_ativa.sql) precisa ter sido aplicado
-- antes — é ele que cria usuario_empresa_ativa e empresa_ativa().
--
-- Só CRIA e SUBSTITUI funções: não altera dado de cliente, dispensa cópia de
-- segurança.
--
...
-- =========================================================
-- CONFERÊNCIA
-- Esperado: decisao_ve_empresa = 1, tolerancia_ligada = false (ainda tolerante,
-- que é o certo nesta etapa) e politicas_que_herdam > 0 — estas são as
-- políticas restritivas que passam a recortar por empresa SEM terem sido
-- tocadas, porque chamam a função que mudou.
--
-- Para fechar a tolerância quando a adoção autorizar:
--   INSERT INTO public.app_config (chave, valor)
--   VALUES ('empresa_ativa_obrigatoria','true')
--   ON CONFLICT (chave) DO UPDATE SET valor = 'true';
-- Para voltar atrás, o mesmo comando com 'false'.
-- =========================================================
SELECT
  (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'perfil_permite_modulo'
      AND pg_get_functiondef(p.oid) ~* 'empresa_ativa')            AS decisao_ve_empresa,
  public.empresa_ativa_obrigatoria()                               AS tolerancia_fechada,
  (SELECT count(*) FROM pg_policies
    WHERE schemaname = 'public' AND policyname LIKE 'perfil_restringe_leitura_%')
                                                                   AS politicas_que_herdam,
  (SELECT count(*) FROM public.usuario_empresa_ativa)              AS empresa_ativa_registradas,
  (SELECT count(*) FROM public.usuarios_base
    WHERE auth_user_id IS NOT NULL AND status::text = 'ativo')     AS usuarios_com_conta;
