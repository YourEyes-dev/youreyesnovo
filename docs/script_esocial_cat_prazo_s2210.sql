...
--
-- DRIFT: afastamento_proximo_dia_util existe no desenvolvimento, mas NAO foi
-- entregue a homologacao/producao (o script quebrava com "function
-- afastamento_proximo_dia_util(uuid, date) does not exist"). Entao a entrega
-- vai autossuficiente: cria/atualiza a funcao antes de usa-la. CREATE OR
-- REPLACE e no-op onde ela ja existe igual.
-- ============================================================================

-- 0) Calendario: proximo dia util (pula fim de semana e feriados cadastrados).
...
AS $fn$
DECLARE
  v_dia date := p_data + 1;   -- "1o dia util SEGUINTE"
  v_i   int := 0;
BEGIN
  WHILE v_i < 30 LOOP
    -- Pula fim de semana (6=sabado, 0=domingo) e feriados cadastrados.
...
-- Conferencia (leve, um caso)
SELECT 'AFAST-030' AS caso, (public.qa_executar_descartavel('qa_caso_afast_030')).situacao::text AS situacao;
