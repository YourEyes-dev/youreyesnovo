-- ============================================================================
-- SCRIPT DE ENTREGA — fecha ISOL-005, ISOL-007 e PER-006
--
-- Os três achados são ANTIGOS: nenhum veio das mudanças desta leva. Estavam
-- abertos porque cada um pedia uma decisão.
--
-- O que este script NÃO faz: não altera nem apaga uma única linha de dado de
-- negócio. Ele cria políticas e funções. Por isso não há cópia de segurança a
-- tirar antes — a regra da casa pede cópia para script que mexe em dado
-- existente, e este não mexe.
--
-- O que ele faz, em ordem:
--   1. ISOL-005 — dá política mínima (só superadmin) às tabelas sensíveis que
--      estão com o isolamento LIGADO e ZERO política. Hoje elas não são lidas
--      por ninguém, nem pelo dono do dado: o script troca "ninguém lê" por "só
--      o superadmin lê". Não afrouxa nada. E protege as cópias backup_* que os
--      próprios scripts de entrega criam, que hoje guardam linha real de
--      cliente sem isolamento nenhum.
--   2. ISOL-007 — cria o registro da travessia de LEITURA do superadmin.
--   3. PER-006 — cria a decisão que distingue o VERBO (ler != escrever).
--
-- Idempotente: rodado duas vezes seguidas sem erro e sem duplicar política.
-- ============================================================================
...
-- ============================================================================
-- CONFERÊNCIA — uma só, que é o que o SQL Editor mostra.
--
-- Esperado: sensiveis_sem_politica = 0, copias_expostas = 0,
--           registra_travessia = true, decisao_com_verbo = true.
-- Se alguma das duas primeiras vier diferente de zero, a coluna
-- erro_tecnico diz quais tabelas ficaram de fora.
-- ============================================================================
WITH sensiveis AS MATERIALIZED (
  SELECT c.relname AS tabela, c.relrowsecurity AS isolada,
         EXISTS (SELECT 1 FROM pg_policies p
                  WHERE p.schemaname = 'public' AND p.tablename = c.relname) AS tem_politica
  FROM pg_class c
  JOIN pg_namespace n ON n.oid = c.relnamespace
...
SELECT
  (SELECT count(*) FROM sensiveis WHERE isolada AND NOT tem_politica)        AS sensiveis_sem_politica,
  (SELECT count(*) FROM sensiveis WHERE tabela LIKE 'backup\_%' AND NOT isolada) AS copias_expostas,
  EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
           WHERE n.nspname = 'public' AND p.prokind = 'f' AND p.proname NOT LIKE 'qa\_%'
             ...
             AND pg_get_functiondef(p.oid) ~* '(is_superadmin|cross.?tenant|outro.?tenant)')
                                                                              AS registra_travessia,
  EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
           WHERE n.nspname = 'public' AND p.prokind = 'f' AND p.proname NOT LIKE 'qa\_%'
             AND p.proname ~* 'perfil.*(permite|pode|acao)'
             AND pg_get_function_arguments(p.oid) ~* '(acao|verbo)')          AS decisao_com_verbo,
  COALESCE((SELECT string_agg(tabela, ', ' ORDER BY tabela) FROM sensiveis
             WHERE (isolada AND NOT tem_politica)
                OR (tabela LIKE 'backup\_%' AND NOT isolada)), '-')           AS erro_tecnico;
