...
-- ============================================================================
-- CONFERÊNCIA — uma só, que é o que o editor mostra.
--
-- Esperado: porteiro_atualizado = 1 e pessoas_que_voltam_a_entrar = 1
-- (a mesma pessoa que a medição de 16/09/2026 encontrou).
--
-- Este script NÃO altera nem apaga nenhuma linha: só substitui uma função.
-- Por isso não há cópia de segurança a fazer antes.
-- ============================================================================
WITH gestao_sem_cadastro AS MATERIALIZED (
  ...
  WHERE EXISTS (SELECT 1 FROM public.user_roles ur
    ...
    AND NOT EXISTS (SELECT 1 FROM public.usuarios_base ub
                     WHERE ub.auth_user_id = p.user_id
                       AND ub.tenant_id = p.tenant_id)
)
SELECT
  (SELECT count(*) FROM pg_proc p
   ...
   WHERE n.nspname = 'public' AND p.prokind = 'f'
     ...
     AND pg_get_functiondef(p.oid) LIKE '%v_status IS NULL%')  AS porteiro_atualizado,
  (SELECT count(*) FROM gestao_sem_cadastro)                   AS pessoas_que_voltam_a_entrar;
