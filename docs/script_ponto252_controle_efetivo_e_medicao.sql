...
-- ENTREGA producao.
--
-- A CHECK chk_ajuste_sem_autoaprovacao (entregue no T3) e cosmetica: compara
-- aprovado_por (auth_user_id) com colaborador_id (usuarios_base.id), espacos de
-- ID diferentes — quase nunca barra o caso real. O controle EFETIVO e o gatilho
-- trg_ponto_ajuste_autoaprovacao, cuja funcao resolve o aprovador em
-- usuarios_base (por id OU CPF) e barra a autoaprovacao real.
--
-- Este script: (1) garante o gatilho efetivo; (2) remede a rotina de QA para
-- medir a presenca do controle EFETIVO e tratar autoaprovacoes anteriores ao
-- controle como informativas (grandfather). So cria/atualiza funcoes e 1
-- gatilho — nao altera nem apaga dado existente. Idempotente. Uma transacao.
-- ============================================================================
...
-- Conferencia (leve)
SELECT 'PONTO-252' AS caso, (public.qa_executar_descartavel('qa_caso_ponto_252')).situacao::text AS situacao;
