...
--
-- O ACHADO (cartao-ponto de agosto/2026): escala das 08:00 as 17:00 com 1h de
-- intervalo declarado por pre-assinalacao. O cartao saiu assim, quase todo dia:
--
--   Marcacoes 08:00 17:00 | H.D. 8:00 | H.N. 9:00 | H.A. 1:00
--   Ocorrencia: "Atraso / saida antecipada"
--
-- H.D. (o trabalhado) estava certo: 9h de janela menos 1h de intervalo. H.N.
-- (a jornada PREVISTA) vinha 9:00 — a janela crua, sem descontar o intervalo.
-- A diferenca virava uma hora de ausencia por dia, e um dia integralmente
...
--
-- E por isso as SEXTAS saiam certas (8:00) e o resto da semana nao: naquele
-- dia a escala tinha a janela de almoco preenchida, nos outros nao. A tela
-- repara configuracoes antigas ao abrir a escala, mas so grava se alguem
-- salvar — por isso a correcao pertence ao motor, nao a tela.
--
...
-- As duas ultimas colunas sao INFORMATIVAS, nao reprovam: dizem o alcance da
-- correcao — quantas declaracoes estao vigentes hoje e quantos colaboradores
-- passam a ter a jornada prevista descontada do intervalo declarado.
-- ---------------------------------------------------------------------------
  ...
  SELECT
    ...
    (SELECT count(DISTINCT a.id)
      ...
      WHERE COALESCE(a.ativa, true) = true
        ...
        AND (a.data_fim IS NULL OR a.data_fim >= CURRENT_DATE)) AS colaboradores_alcancados
)
SELECT funcao_ok, le_declaracao, declaracoes_vigentes, colaboradores_alcancados,
       CASE WHEN funcao_ok AND le_declaracao THEN 'OK' ELSE 'CONFERIR' END AS erro_tecnico
FROM x;
