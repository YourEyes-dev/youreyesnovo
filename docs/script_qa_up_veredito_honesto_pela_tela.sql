-- ============================================================================
-- SCRIPT DE ENTREGA — QA Usuários & Permissões: veredito honesto pela tela
--
-- O que este script faz: só mexe em rotinas de QA. Nenhuma tabela, nenhuma
-- política, nenhum dado de cliente é tocado — portanto não há nada para
-- guardar antes.
--
-- Por que ele existe: a bateria disparada pelo BOTÃO da tela entra por uma
-- função com privilégio do dono (qa_disparar_bateria, que precisa do
-- privilégio para conferir o superadmin). O PostgreSQL proíbe trocar de papel
-- lá dentro, e doze casos deste módulo só provam alguma coisa calçando os
-- sapatos de um usuário comum. Duas dessas rotinas QUEBRAVAM por causa de um
-- RESET ROLE solto; as outras dez diziam "este ambiente não deixa", sem
-- nomear a causa nem o caminho. Agora as doze se declaram sem veredito, com a
-- mesma explicação e com o comando que dá o veredito completo.
--
-- Idempotente: é tudo CREATE OR REPLACE. Rodar duas vezes não muda nada.
-- ============================================================================
...
-- ============================================================================
-- CONFERÊNCIA — o editor só mostra o último resultado, então é uma só.
--
-- Esperado: apoios_criados = 2, rotinas_com_a_mensagem_unica = 12,
--           rotinas_com_o_texto_antigo = 0, duas_sem_reset_solto = 2.
-- ============================================================================
WITH rotinas AS MATERIALIZED (
...
SELECT
  (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname IN ('qa_up_motivo_sem_papel', 'qa_up_voltar_papel'))         AS apoios_criados,
  (SELECT count(*) FROM rotinas WHERE corpo LIKE '%qa_up_motivo_sem_papel%')     AS rotinas_com_a_mensagem_unica,
  (SELECT count(*) FROM rotinas WHERE corpo LIKE '%Este ambiente não deixa%')    AS rotinas_com_o_texto_antigo,
  (SELECT count(*) FROM rotinas
    WHERE proname IN ('qa_caso_lib_005', 'qa_caso_nac_006')
      AND corpo NOT LIKE '%RESET ROLE%')                                         AS duas_sem_reset_solto;
