-- =========================================================
-- QA — Documentacao de Testes: Usuarios, Niveis de Acesso,
--      Liberacoes e Permissoes
--
-- Modulo novo: infraestrutura-auth/usuarios-permissoes.
-- Origem: documento de QA "Usuarios, Niveis de Acesso, Liberacoes e
-- Permissoes" (autor: agente QA senior), que desenha a prova do controle de
-- acesso: quem e o usuario, a quais empresas esta vinculado, qual nivel tem
-- EM CADA vinculo, e o que efetivamente pode ver e fazer.
--
-- O modelo REAL do YourEyes ja sustenta o desenho do documento:
--   · vinculo usuario x empresa ... usuario_perfil_vinculos (usuario_id,
--                                   empresa_id, perfil_id, ativo, expira_em)
--   · nivel de acesso ............. perfis_acesso + perfil_permissoes
--                                   (parametrizado em tabela, nao em codigo)
--   · liberacao pontual ........... perfil_excecoes (tipo adicional, expira_em)
--   · auditoria ................... perfil_audit_log
--   · ciclo de vida do usuario .... usuarios_base.status
--   · papeis de sistema ........... user_roles + has_minimum_role
--
-- Renomeacao de prefixos em relacao ao documento (o codigo e UNICO no
-- sistema inteiro e os prefixos do documento ja estavam ocupados):
--   RLS-001..008 do documento  ->  ISOL-001..008  (RLS-* = modulo Isolamento RLS)
--   ESC-001..006 do documento  ->  PRIV-001..006  (ESC-* = modulo Escalas)
-- As demais familias entram com o prefixo do documento: USR, VIN, NAC, PER,
-- LIB, CTX, SES.
--
-- Nivel: 'api' para o que se prova no banco (o documento chama pgTAP e
-- API/dados) e 'e2e' para as jornadas que so a tela prova. Regra da casa:
-- nao se empurra pra tela o que se prova no banco.
--
-- Status 'aprovado': o motor (qa_rodar_bateria) so enxerga caso aprovado.
-- Enquanto nao houver rotina qa_caso_*, cada caso aparece honestamente como
-- "nao_implementado" na bateria — que e exatamente o estado de hoje e a
-- fila da proxima etapa (motor + Cypress).
--
-- Esta migration SO documenta: nao cria nem altera regra de acesso.
...
-- ---------------------------------------------------------
-- 1) Modulo novo na arvore da Documentacao de Testes
-- ---------------------------------------------------------
DO $mod$
DECLARE v_sec uuid;
BEGIN
  ...
  IF v_sec IS NULL THEN
    RAISE EXCEPTION 'Bloco infraestrutura-auth nao encontrado na arvore de QA.';
  END IF;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Modulo infraestrutura-auth/usuarios-permissoes nao encontrado.'; END IF;
