-- =========================================================
-- QA — Motor: rotinas do módulo Usuários & Permissões
--
-- Executa no motor SQL os casos de nível 'api' documentados em
-- infraestrutura-auth/usuarios-permissoes (migration 20260916160000).
--
-- COMO O MOTOR PROTEGE ESTAS ROTINAS (não precisamos repetir nada):
--   qa_executar_descartavel liga o modo de teste, exige que esteja ligado,
--   roda a rotina e DESFAZ tudo que ela escreveu. Por isso as rotinas podem
--   montar o cenário à vontade: nada sobra na base.
--   A trava qa_guarda_cercado recusa qualquer escrita fora do tenant do
--   cercado, mesmo que a rotina erre o alvo.
--
-- DUAS ARMADILHAS DO SCHEMA QUE AS FIXTURES PRECISAM DESARMAR:
--   1. perfil_permite_modulo devolve TRUE de saída para superadmin, para
--      papel >= manager e para tipo_usuario administrador/gestor. Um usuário
--      de teste com qualquer um desses atalhos passaria em tudo sem provar
--      nada. As fixtures usam tipo_usuario 'colaborador' e NENHUM user_roles.
--   2. o gatilho trg_vincular_perfil_padrao anexa sozinho um vínculo de
--      perfil padrão a todo usuário não-administrador recém-criado. Se ele
--      ficasse, o cenário teria um vínculo a mais do que o declarado e casos
--      de contagem (VIN-002, VIN-007) mediriam outra coisa. A fixture apaga
--      esse vínculo automático logo após criar o usuário.
--
-- SOBRE O PLACAR: várias rotinas devem voltar 'falhou' na primeira rodada.
-- Isso é o ACHADO, não defeito da rotina — o motor de QA existe para
-- encontrar, não para confirmar. As lacunas que já se conhecem pela leitura
-- do código estão anotadas caso a caso, e a correção do produto é decisão
-- separada, que não vem nesta migration.
--
-- Idempotente: só CREATE OR REPLACE FUNCTION e upsert em qa_implementacoes.
-- Não altera nenhuma regra de acesso do sistema.
-- =========================================================
...
DO $pre$
BEGIN
  IF to_regtype('public.qa_retorno') IS NULL THEN
    RAISE EXCEPTION 'qa_retorno não existe — rode as migrations do motor de QA antes.';
  END IF;
  IF to_regclass('public.qa_implementacoes') IS NULL THEN
    RAISE EXCEPTION 'qa_implementacoes não existe — rode as migrations do motor de QA antes.';
  END IF;
  IF to_regclass('public.usuario_perfil_vinculos') IS NULL THEN
    RAISE EXCEPTION 'usuario_perfil_vinculos não existe — schema de perfis ausente.';
  END IF;
END $pre$;
