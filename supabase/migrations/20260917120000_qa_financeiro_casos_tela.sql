-- =========================================================
-- QA — Financeiro ganha documentação de TELA (nível e2e) + ponte
--
-- O módulo Financeiro (folha, benefícios, provisões, eSocial...) não tinha
-- documentação de TELA. Mesmo passo de Metas/Férias/Colaboradores: a bateria
-- estrutural de tela (o módulo monta, as abas abrem, os modais principais —
-- Novo Período e Novo Tipo de Benefício — aparecem, e os vazios orientam).
--
-- Prefixo -TELA- não colide com as sondas de motor (FOLHA-*, RUB-*, etc.).
-- Todos os casos são DATA-INDEPENDENTES (a tela monta e as abas/modais abrem
-- mesmo na ilha vazia). Não exigem fixtures.
--
-- Duas entregas: esta migration (robô aplica no staging) e
-- docs/script_financeiro_casos_tela_homologacao.sql (SQL Editor da homologação
-- e depois produção). Idempotente: ON CONFLICT (codigo) DO NOTHING.
-- =========================================================

SET lock_timeout = '10s';

-- ══════════════════════════════════════════════════════════
-- FINANCEIRO  (financeiro)  — rota /financeiro
-- ══════════════════════════════════════════════════════════
DO $doc$
DECLARE v_mod uuid; v_antes int; v_depois int;
BEGIN
  SELECT id INTO v_mod FROM public.qa_modulos WHERE path = 'financeiro';
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo financeiro não encontrado.'; END IF;
  ...
  RAISE NOTICE 'Financeiro (tela): antes=%, depois=% (esperado +10)', v_antes, v_depois;
END $doc$;

-- ══════════════════════════════════════════════════════════
-- Ponte de cobertura e2e: liga cada it() de financeiro.cy.ts ao caso.
