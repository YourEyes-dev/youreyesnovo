-- ============================================================================
-- PONTO — a selfie no link passa a respeitar a configuração da empresa
--
-- O PROBLEMA
--   Existe a configuração ponto_configuracao.exigir_selfie_link (padrão true),
--   mas o registro por link compartilhado (CPF) IGNORAVA ela: a função
--   registrar_ponto_externo_cpf recusava qualquer registro sem selfie ("É
--   obrigatório tirar a selfie…") como trava anti-fraude do CPF. Empresas que
--   desmarcavam a opção continuavam obrigadas a tirar selfie — a config não
--   tinha efeito nenhum.
--
-- A CORREÇÃO
--   A selfie passa a ser exigida SOMENTE quando exigir_selfie_link = true.
--   Quando a empresa desmarca, o ponto é registrado sem selfie. Como o padrão
--   é true (e sem linha de config o COALESCE devolve true), o comportamento de
--   quem NÃO mexeu na configuração continua idêntico: selfie exigida.
--
--   As funções de resolução do link (buscar_ponto_link_por_token e
--   buscar_colaborador_por_cpf) passam a devolver 'exigir_selfie' para a tela
--   saber se deve abrir a câmera e travar o botão.
--
--   ponto_configuracao.tenant_id é UNIQUE (uma linha por empresa/tenant), então
--   a leitura é um subselect escalar simples.
--
-- Idempotente: CREATE OR REPLACE das três funções. Nada de dado é alterado.
-- ============================================================================

-- 1) buscar_ponto_link_por_token — devolve exigir_selfie ----------------------
...
-- 2) buscar_colaborador_por_cpf — devolve exigir_selfie -----------------------
...
-- 3) registrar_ponto_externo_cpf — selfie condicionada à config ---------------
