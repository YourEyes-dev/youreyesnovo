-- ============================================================================
-- Ouvidoria: link público (sem login) para manifestações
--
-- Espelha o padrão do link COMPARTILHADO do ponto: um único token por tenant,
-- cliente anônimo (supabasePublic) chamando RPCs SECURITY DEFINER liberadas para
-- o papel anon. A tabela public.ouvidoria continua fechada por RLS para
-- authenticated; o anon nunca toca a tabela direto — só pelas RPCs abaixo.
--
-- Decisões (confirmadas com o dono do produto):
--  - Identificação por CPF é DECLARATÓRIA: se o CPF casar com um colaborador,
--    preenche nome/empresa; se não casar, aceita mesmo assim (não bloqueia).
--  - Anônimo: nada de identidade é gravado.
--  - Protocolo de acompanhamento é gerado e devolvido no envio.
-- ============================================================================

SET lock_timeout = '10s';

-- 1) Colunas novas em public.ouvidoria (aditivas, idempotentes) ---------------
...
-- Protocolo único por tenant (quando houver).
...
-- No máximo UM link por tenant (mesmo espírito do link compartilhado do ponto).
...
-- Casa por dígitos contra admissoes concluídas do tenant. NÃO exige bate_ponto
-- (não é ponto). Uso interno pelas RPCs SECURITY DEFINER — não liberado a anon.
...
-- 5) buscar_ouvidoria_link_por_token: valida o link e devolve o nome da empresa
...
-- 6) buscar_colaborador_ouvidoria_por_cpf: identificação declaratória ---------
-- Nunca dá erro por "não encontrado": devolve encontrado=false e o front segue
-- com o nome digitado. Só erra se o link for inválido.
...
-- 7) registrar_manifestacao_externa: grava a manifestação via link público ----
...
BEGIN
  ...
  -- Identificação declaratória (só quando NÃO anônimo).
  IF COALESCE(p_anonimo, false) = false THEN
    ...
    END IF;
  ELSE
    -- Anônimo: descarta qualquer identidade.
    v_cpf_digits := NULL;
    v_nome := NULL;
    v_email := NULL;
    v_empresa_id := NULL;
  ...
  -- Roteamento configurado (mesma regra do envio autenticado).
  ...
  -- Protocolo único por tenant (poucas tentativas bastam).
  LOOP
...
-- 8) buscar_ouvidoria_link_por_ponto_token: resolve o link da Ouvidoria a partir
-- do token do ponto (para o botão de Ouvidoria na tela pública de ponto).
